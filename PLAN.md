...
| 43 | Computer-course grading | **Option B from `V2_RUNNER_DESIGN.md`, piloted on Command Line and Git only: a downloadable local checker the learner runs on their own computer, whose pasted report is shown as learner-reported practice.** A local report never awards XP, completion, review, evidence, or certificate credit, and moving past a local step never counts it as done | Owner's call, 2026-09-24. It is the only option that checks real files and real Git state without hosted execution (decision 9) or a new dependency. The checker is repository-owned Node code (`apps/web/tools/local-checker.mjs`), served at `/downloads/codedaddy-check.mjs` with the lesson checks embedded as data. The same file is the authoring gate: `npm run check:content` replays each step's solution commands in a throwaway folder and requires the checks to fail before and pass after. The commands and checks for all 18 projects are fixed in `apps/web/tools/cli-git-plan.mjs`; the local model writes only the lesson text. Because a pasted report can be edited, it proves nothing about the learner, which is why it stays practice-only. Node.js Fundamentals and later courses stay ungraded until the pilot is proven and a trust model for completion is chosen. A course whose steps are all local never reaches 100% completion; decision 44 removes course locks so this blocks nothing |
| 44 | Course locks | **No course is ever locked. Every course with lessons opens from the map and from its URL at any time.** A course's `requires` list now only suggests an order, shown on the map as "Best after" the earlier course | Owner's call, 2026-09-24. Decision 43 made Command Line and Git practice-only, so it can never reach 100% and would have locked Node.js and every later Program C course for good. Removing locks everywhere solves that and treats learners as adults choosing their own path. XP, completion, evidence, and certificates are unchanged: the five capstones still require their earlier independent activities, because that ordering is part of the certificate, not navigation |
| 45 | Node.js and later computer courses | **Extend decision 43's local checker to Node.js Fundamentals, then Building APIs, Auth and Security, and Full-Stack Integration, one course at a time. These courses teach Node's built-in modules (`node:fs`, `node:http`, `node:crypto`, `node:sqlite`) instead of npm packages, with one closing project that shows the same work in Express.** Results stay learner-reported practice | Owner's call, 2026-09-24. Built-ins need no install and no network, so every step can be checked offline on a learner's machine and replayed in the authoring gate without installing packages. The checker gains three plain-data checks that run the learner's own script with `node` under a short time limit (`local-node-prints`, `local-node-stderr`, `local-node-exit-code`). As with Command Line and Git, the code and checks for every step are fixed in a plan file (`apps/web/tools/node-basics-plan.mjs`) that the gate replays; the local model writes only the lesson text. The cost is less time with Express, which is the tool most job listings name |
| 46 | Full-Stack front end and checks | **Full-Stack Integration uses React and Vite installed from npm on the learner's computer, pinned to exact versions (react and react-dom 19.3.0, vite 8.3.1, @vitejs/plugin-react 6.1.1). The checker verifies by running the learner's own `npm run build`, rendering one component to HTML in Node with the learner's own react-dom/server and plain-data props, and sending real requests to the learner's server. No browser automation.** | Owner's call, 2026-09-25, choosing the tools job listings name over a no-install course. These packages live only in the learner's project and the content gate's cache; none is added to this repository's package.json, and nothing new ships to the website. The cost: a learner needs internet once per project for npm install (about 42 MB of packages, cached by npm afterwards), and clicks, typing, and layout are taught but never checked. The content gate installs each pinned package set once into a shared cache and links projects to it |
...
| Full-Stack Integration | 400 | Own computer | Connecting a React front end to the learner's own API, deployment, and the full-stack capstones. React and Vite from npm with pinned versions (decision 46); checked by building, rendering components to HTML in Node, and real HTTP requests, never by clicking in a browser |
