import { runLearnerNosql } from "./nosql-runner";
import { NOSQL_KINDS, runNosqlTest, type NosqlTestSpec } from "./nosql-assertions";
...
export async function gradeStep(
  ...
  // One database per grading run, built from the step's seed and thrown away
  // with the frame. Every SQL assertion in the step reads the same result.
  const sqlRun: SqlRunResult | null = step.tests.some((t) => SQL_KINDS.has(t.kind))
    ? await runLearnerSql(step.sqlSeed ?? "", files["query.sql"] ?? "")
    : null;

  const nosqlRun = step.tests.some(t => NOSQL_KINDS.has(t.kind))
    ? await runLearnerNosql(step.nosqlSeed ?? {}, files["query.json"] ?? "")
    : null;
  ...
  try {
    ...
    return step.tests.map((spec) => {
      ...
      if (SQL_KINDS.has(spec.kind) && sqlRun) {
        return runSqlTest(spec as SqlTestSpec, sqlRun);
      }

      if (NOSQL_KINDS.has(spec.kind) && nosqlRun) {
        return runNosqlTest(spec as NosqlTestSpec, nosqlRun);
      }
