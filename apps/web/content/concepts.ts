...
export const concepts: Record<string, Concept> = {
  ...
  "i-element": {
    id: "i-element",
    term: "i element",
    definition: "The i element sets text apart from its surrounding voice, such as a scientific name or a term from another language.",
    analogy: "Like a field guide setting a species name apart from the sentence around it.",
    visual: { kind: "diagram", diagram: { alt: "A scientific mosquito name is set apart from the surrounding health notice.", columns: 3, nodes: [{ id: "words", label: "Aedes aegypti", note: "scientific name", tone: "ghost" }, { id: "tag", label: "i", note: "different voice", tone: "accent" }, { id: "result", label: "Set apart", note: "usually italic", tone: "box" }], arrows: [{ from: "words", to: "tag" }, { from: "tag", to: "result" }] } },
    proof: "You will set the mosquito's scientific name apart from the rest of the notice.",
  },
  "b-element": {
    id: "b-element",
    term: "b element",
    definition: "The b element draws attention to text without saying that the text is more important.",
    analogy: "Like printing a route name in heavier ink so commuters can scan it quickly.",
    visual: { kind: "diagram", diagram: { alt: "The route name Divisoria is visually set apart without changing its importance.", columns: 3, nodes: [{ id: "words", label: "Divisoria", note: "route name", tone: "ghost" }, { id: "tag", label: "b", note: "draw attention", tone: "accent" }, { id: "result", label: "Easy to spot", note: "same importance", tone: "box" }], arrows: [{ from: "words", to: "tag" }, { from: "tag", to: "result" }] } },
    proof: "You will make a jeepney destination easier to spot without using semantic importance.",
  },
  "sub-element": {
    id: "sub-element",
    term: "sub element",
    definition: "The sub element marks text that belongs below the normal line, such as the 2 in H2O.",
    analogy: "Like lowering one small tile beneath the rest of a formula.",
    visual: { kind: "diagram", diagram: { alt: "The number 2 sits below the letters H and O in the water formula.", columns: 3, nodes: [{ id: "words", label: "H 2 O", note: "water formula", tone: "ghost" }, { id: "tag", label: "sub", note: "lower the 2", tone: "accent" }, { id: "result", label: "H₂O", note: "correct formula", tone: "box" }], arrows: [{ from: "words", to: "tag" }, { from: "tag", to: "result" }] } },
    proof: "You will place the 2 below the line in the formula H2O.",
  },
  "em-element": {
    id: "em-element",
    term: "em element",
    definition: "The em element marks a word said with stress, changing the meaning of the sentence.",
    analogy: "Like stressing one word when speaking so the listener understands the intended meaning.",
  ...
  "span-element": {
    ...
    analogy: "Like putting a small label around part of a sentence so CSS or JavaScript can find it later.",
  ...
  "thead-element": {
    ...
    analogy: "Like the labelled top row of a paper stock sheet, kept separate from the entries below.",
    visual: { kind: "diagram", diagram: { alt: "A medicine heading row is grouped inside the head of a stock table.", columns: 3, nodes: [{ id: "words", label: "Medicine", note: "heading cell", tone: "ghost" }, { id: "tag", label: "thead", note: "group header rows", tone: "accent" }, { id: "result", label: "Table head", note: "before stock rows", tone: "box" }], arrows: [{ from: "words", to: "tag" }, { from: "tag", to: "result" }] } },
    proof: "You will place a valid heading row inside a table head.",
  },
  "tbody-element": {
    id: "tbody-element",
    term: "tbody element",
    definition: "The tbody element groups the main rows of a table.",
    analogy: "Like the middle of an application log where each applicant gets one row.",
    visual: { kind: "diagram", diagram: { alt: "An applicant row is grouped inside the body of an application table.", columns: 3, nodes: [{ id: "words", label: "Maria Santos", note: "data cell", tone: "ghost" }, { id: "tag", label: "tbody", note: "group data rows", tone: "accent" }, { id: "result", label: "Table body", note: "main records", tone: "box" }], arrows: [{ from: "words", to: "tag" }, { from: "tag", to: "result" }] } },
    proof: "You will place a valid applicant row inside a table body.",
  },
  "tfoot-element": {
    id: "tfoot-element",
    term: "tfoot element",
    definition: "The tfoot element groups the summary rows of a table, such as a total.",
    analogy: "Like the total line at the bottom of a handwritten receipt.",
    visual: { kind: "diagram", diagram: { alt: "A total row is grouped inside the footer of a price table.", columns: 3, nodes: [{ id: "words", label: "Total: ₱100.00", note: "summary cell", tone: "ghost" }, { id: "tag", label: "tfoot", note: "group summary rows", tone: "accent" }, { id: "result", label: "Table footer", note: "price summary", tone: "box" }], arrows: [{ from: "words", to: "tag" }, { from: "tag", to: "result" }] } },
    proof: "You will place a valid total row inside a table footer.",
  },
  "u-element": {
    id: "u-element",
    term: "u element",
    definition: "The u element marks text with a non-textual annotation, such as a spelling error. Browsers usually underline it.",
    analogy: "A red pen underlining a mistake in your homework.",
    visual: { kind: "diagram", diagram: { alt: "A draft health notice underlines the misspelled word vacine.", columns: 3, nodes: [{ id: "words", label: "vacine", note: "draft misspelling", tone: "ghost" }, { id: "tag", label: "u", note: "spelling annotation", tone: "accent" }, { id: "result", label: "Underlined", note: "marked for correction", tone: "box" }], arrows: [{ from: "words", to: "tag" }, { from: "tag", to: "result" }] } },
    proof: "You will annotate one misspelled word in a draft notice.",
  },
  "sup-element": {
    id: "sup-element",
    term: "sup element",
  ...
  "bdi-element": {
    ...
    analogy: "Like putting an unknown player name in its own lane so it cannot rearrange the score beside it.",
    ...
    proof: "You will isolate a player name so its writing direction cannot rearrange the surrounding score.",
  },
  "react-component": {
    id: "react-component",
    term: "React component",
    definition: "A React component is a function that describes one part of a page.",
    analogy: "Like one barangay desk with one clear job inside a larger service centre.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<main><section><h2>Health Centre</h2><p>Open today until 5 PM.</p></section></main>",
        "styles.css": reactDemoCss,
      },
      caption: "A service-card component appears as one working part of the page.",
    },
    proof: "You will create App, then CodeDaddy will ask React to render it.",
  },
  "react-element": {
    id: "react-element",
    term: "React element",
    definition: "A React element is a description of what React should place on the page.",
    analogy: "Like a service slip that tells the barangay desk which sign to put up.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<main><h1>Barangay Help Desk</h1></main>",
        "styles.css": reactDemoCss,
      },
      caption: "The element description becomes this real heading in the browser.",
    },
    proof: "You will return a heading element and see React render it.",
  },
  "react-component-composition": {
    id: "react-component-composition",
    term: "component composition",
    definition: "Component composition means placing one component inside another component.",
    analogy: "Like placing one service desk inside the larger barangay help centre.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<main><h1>Barangay Services</h1><section><h2>Health Centre</h2><p>Open today.</p></section></main>",
        "styles.css": reactDemoCss,
      },
      caption: "The service card sits inside the larger barangay-services page.",
    },
    proof: "You will make App include the ServiceCard component.",
  },
  "react-nested-element": {
    id: "react-nested-element",
    term: "nested React element",
    definition: "A nested React element is an element placed inside another element.",
    analogy: "Like putting a service name sign inside the service card that owns it.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<section><h2>Health Centre</h2></section>",
        "styles.css": reactDemoCss,
      },
      caption: "The heading is visibly contained by its section.",
    },
    proof: "You will place a heading element inside a section element.",
  },
  "react-list-rendering": {
    id: "react-list-rendering",
    term: "list rendering",
    definition: "List rendering turns each value in an array into a React element.",
    analogy: "Like turning every name in a barangay logbook into its own notice line.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<main><h2>Notices</h2><ul><li>Water interruption</li><li>Cleanup drive</li></ul></main>",
        "styles.css": reactDemoCss,
      },
      caption: "Two array values appear as two real list items.",
    },
    proof: "You will map two notice values into two list items.",
  },
  "react-key": {
    id: "react-key",
    term: "React key",
    definition: "A React key is a stable value that identifies one item in a rendered list.",
    analogy: "Like a queue number that keeps one request distinct when the line changes.",
    visual: {
      kind: "diagram",
      diagram: {
        alt: "Each notice receives its own stable key.",
        columns: 3,
        nodes: [
          { id: "notice", label: "notice", note: "one value", tone: "ghost" },
          { id: "key", label: "key", note: "stable identity", tone: "accent" },
          { id: "item", label: "li", note: "tracked item", tone: "box" },
        ],
        arrows: [
          { from: "notice", to: "key" },
          { from: "key", to: "item" },
    ...
    proof: "You will use each notice value as the key for its list item.",
  },
  "react-conditional-rendering": {
    id: "react-conditional-rendering",
    term: "conditional rendering",
    definition: "Conditional rendering shows different page content when a condition changes.",
    analogy: "Like one office sign showing Open now by day and Closed after hours.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<main><p id=\"status\">Open now</p><button type=\"button\">Change condition</button></main>",
        "styles.css": reactDemoCss,
        "script.js": "const status=document.querySelector('#status');document.querySelector('button').addEventListener('click',()=>{status.textContent=status.textContent==='Open now'?'Closed':'Open now';});",
      },
      caption: "Use the button to make one condition show either status.",
    },
    proof: "You will make one boolean choose the office status text.",
  },
  "react-state": {
    id: "react-state",
    term: "React state",
    definition: "React state is a value a component remembers while the learner uses the page.",
    analogy: "Like a queue desk remembering how many people are currently waiting.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<main><p>Queue count: <strong id=\"count\">0</strong></p><button type=\"button\">Add person</button></main>",
        "styles.css": reactDemoCss,
        "script.js": "let count=0;const output=document.querySelector('#count');document.querySelector('button').addEventListener('click',()=>{count+=1;output.textContent=String(count);});",
      },
      caption: "The page remembers the count as you add people.",
    },
    proof: "You will create count state that begins at zero.",
  },
  "react-event-handler": {
    id: "react-event-handler",
    term: "event handler",
    definition: "An event handler is a function React runs after a user action such as a click.",
    analogy: "Like a queue clerk adding one only when the next person arrives at the desk.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<main><button type=\"button\">Add person</button><p id=\"result\" role=\"status\">Waiting for a click.</p></main>",
        "styles.css": reactDemoCss,
        "script.js": "document.querySelector('button').addEventListener('click',()=>{document.querySelector('#result').textContent='The click handler ran.';});",
      },
      caption: "Clicking the button runs its handler and changes the result.",
    },
    proof: "You will make one button click increase the queue count.",
  },
  "react-state-updater": {
    id: "react-state-updater",
    term: "state updater function",
    definition: "A state updater function calculates the next state from the current state.",
    analogy: "Like reading the current queue number before writing the next number.",
    visual: {
      kind: "diagram",
      diagram: {
        alt: "The current count passes through an updater and becomes the next count.",
        columns: 3,
        nodes: [
          { id: "current", label: "current", note: "old count", tone: "ghost" },
          { id: "updater", label: "+ 1", note: "calculate next", tone: "accent" },
          { id: "next", label: "next count", note: "saved state", tone: "box" },
        ],
        arrows: [
          { from: "current", to: "updater" },
          { from: "updater", to: "next" },
        ],
      },
    },
    proof: "You will update count from the current value passed into setCount.",
  },
  "react-controlled-input": {
    id: "react-controlled-input",
    term: "controlled input",
    definition: "A controlled input gets its value from state and sends every edit back to state.",
    analogy: "Like a visitor log where the displayed name and the saved name stay identical.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<main><label for=\"visitor\">Visitor name</label><br><input id=\"visitor\"><p>Name in state: <strong id=\"name\"></strong></p></main>",
        "styles.css": reactDemoCss,
        "script.js": "const input=document.querySelector('#visitor');input.addEventListener('input',()=>{document.querySelector('#name').textContent=input.value;});",
      },
      caption: "Type a name; the saved value and visible input stay together.",
    },
    proof: "You will type a visitor name and see state update the page.",
  },
  "react-effect": {
    id: "react-effect",
    term: "React effect",
    definition: "A React effect runs work that connects a component to something outside its returned elements.",
    analogy: "Like changing the sign outside an office after the message inside is ready.",
    visual: {
      kind: "diagram",
      diagram: {
        alt: "React renders the heading, then an effect updates the document title.",
        columns: 3,
        nodes: [
          { id: "render", label: "Render", note: "show heading", tone: "ghost" },
          { id: "effect", label: "useEffect", note: "run after", tone: "accent" },
          { id: "title", label: "document.title", note: "browser value", tone: "box" },
        ],
        arrows: [
          { from: "render", to: "effect" },
          { from: "effect", to: "title" },
        ],
      },
    },
    proof: "You will use an effect to copy message into the browser document title.",
  },
  "react-effect-dependency": {
    id: "react-effect-dependency",
    term: "effect dependency",
    definition: "An effect dependency is a value listed beside an effect to declare what that effect uses.",
    analogy: "Like writing message on the work slip for the outside sign.",
    visual: {
      kind: "diagram",
      diagram: {
        alt: "The message value is listed beside the effect that uses it.",
        columns: 3,
        nodes: [
          { id: "message", label: "message", note: "listed input", tone: "ghost" },
          { id: "effect", label: "useEffect", note: "reads message", tone: "accent" },
          { id: "title", label: "title", note: "effect output", tone: "box" },
        ],
        arrows: [
          { from: "message", to: "effect", label: "listed" },
          { from: "effect", to: "title" },
        ],
      },
    },
    proof: "You will list message beside the effect that reads it.",
  },
  "react-ref": {
    id: "react-ref",
    term: "React ref",
    definition: "A React ref is an object that can point to a rendered page element.",
    analogy: "Like keeping the exact desk number so you can go straight to that desk later.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<main><label for=\"search\">Search services</label><br><input id=\"search\"><button type=\"button\">Focus search</button></main>",
        "styles.css": reactDemoCss,
        "script.js": "const search=document.querySelector('#search');document.querySelector('button').addEventListener('click',()=>search.focus());",
      },
      caption: "Press the button; the saved element link moves focus to the search field.",
    },
    proof: "You will connect a ref to the search input and focus that exact field.",
  },
  "react-props": {
    id: "react-props",
    term: "props",
    definition: "Props are values passed to a React element or component.",
    analogy: "Like details written on a request slip before it reaches the service desk.",
    visual: {
      kind: "live-demo",
      files: {
        "index.html": "<main><h1 class=\"help-title\">Barangay Help Desk</h1></main>",
        "styles.css": `${reactDemoCss}.help-title{color:#1d4ed8;border-left:6px solid #1d4ed8;padding-left:10px}`,
      },
