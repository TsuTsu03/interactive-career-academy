...
const HEALTH_CENTER_NOTICE_INTRO_BODY = `    <h2>Important Notice</h2>
    <p>The notice names the mosquito that can spread dengue.</p>\n`;
...
const JEEPNEY_ROUTE_INTRO_BODY = `    <h2>Jeepney Route Notice</h2>
    <p>The route name should be easy to spot without adding extra importance.</p>\n`;
...
const SARI_SARI_STORE_HEADING_BODY = `    <h2>Water Refill Label</h2>\n`;
const SARI_SARI_STORE_INTRO_BODY = `    <h2>Water Refill Label</h2>
    <p>The label needs the chemical formula for water.</p>\n`;
const SARI_SARI_STORE_ELEMENT_SLOT = slotPage(`${SARI_SARI_STORE_INTRO_BODY}    @@SLOT@@\n`, "@@SLOT@@");
const SARI_SARI_STORE_ELEMENT_BODY = `${SARI_SARI_STORE_INTRO_BODY}    <p>Water: H<sub></sub>O</p>\n`;
...
const STOCK_LIST_INTRO_BODY = `    <h2>Pharmacy Stock List</h2>
    <p>The table separates medicine headings from stock rows.</p>
    <table>
${TABLE_SLOT_INDENT}
    </table>\n`;
const STOCK_LIST_ELEMENT_SLOT = slotPage(STOCK_LIST_INTRO_BODY.replace("      \n", "      @@SLOT@@\n"), "@@SLOT@@");
const STOCK_LIST_ELEMENT_BODY = STOCK_LIST_INTRO_BODY.replace("      \n", "      <thead><tr><th></th></tr></thead>\n");
...
const ID_APPLICATION_INTRO_BODY = `    <h2>Barangay ID Application</h2>
    <p>The table body will hold one row for each applicant.</p>
    <table>
${TABLE_SLOT_INDENT}
    </table>\n`;
const ID_APPLICATION_ELEMENT_SLOT = slotPage(ID_APPLICATION_INTRO_BODY.replace("      \n", "      @@SLOT@@\n"), "@@SLOT@@");
const ID_APPLICATION_ELEMENT_BODY = ID_APPLICATION_INTRO_BODY.replace("      \n", "      <tbody><tr><td></td></tr></tbody>\n");
...
const STORE_PRICE_INTRO_BODY = `    <h2>Sari-Sari Store Price</h2>
    <p>The table footer will hold the price summary.</p>
    <table>
${TABLE_SLOT_INDENT}
    </table>\n`;
const STORE_PRICE_ELEMENT_SLOT = slotPage(STORE_PRICE_INTRO_BODY.replace("      \n", "      @@SLOT@@\n"), "@@SLOT@@");
const STORE_PRICE_ELEMENT_BODY = STORE_PRICE_INTRO_BODY.replace("      \n", "      <tfoot><tr><td></td></tr></tfoot>\n");
...
const HEALTH_CENTRE_INTRO_BODY = `    <h2>Barangay Health Centre</h2>
    <p>The draft notice contains one misspelled word.</p>\n`;
...
const FARE_TABLE_B2 = `    <article>\n      <h2>Tricycle Fare Update</h2>\n    </article>\n`;
const FARE_TABLE_B3 = `    <article>\n      <h2>Tricycle Fare Update</h2>\n      <p>Effective immediately, all rates increased by Php5.</p>\n    </article>\n`;
...
/* composition: notice-card */
const CENTRE_NOTICE_ROOT_SLOT = slotPage("    @@SLOT@@\n", "@@SLOT@@");
const CENTRE_NOTICE_B1 = `    <section></section>\n`;
const CENTRE_NOTICE_B2 = `    <section>\n      <h2>Vaccination Alert</h2>\n    </section>\n`;
const CENTRE_NOTICE_B3 = `    <section>\n      <h2>Vaccination Alert</h2>\n      <p>All residents welcome for free shots. Next session tomorrow at 8 AM.</p>\n    </section>\n`;

/* composition: notice-card */
const BOARD_NOTICE_ROOT_SLOT = slotPage("    @@SLOT@@\n", "@@SLOT@@");
const BOARD_NOTICE_B1 = `    <section></section>\n`;
...
/* composition: notice-card */
const MENU_NOTICE_ROOT_SLOT = slotPage("    @@SLOT@@\n", "@@SLOT@@");
const MENU_NOTICE_B1 = `    <section></section>\n`;
...
const references = {
  ...
  "health-center-notice-i-text": { estimatedMinutes: 4, solution: solved(`${HEALTH_CENTER_NOTICE_INTRO_BODY}    <i>Aedes aegypti</i>\n`) },
  ...
  "jeepney-route-b-text": { estimatedMinutes: 4, solution: solved(`${JEEPNEY_ROUTE_INTRO_BODY}    <b>Divisoria</b>\n`) },
  ...
  "sari-sari-store-sub": { estimatedMinutes: 5, solution: solvedSlot(SARI_SARI_STORE_ELEMENT_SLOT, "<p>Water: H<sub></sub>O</p>") },
  "sari-sari-store-sub-text": { estimatedMinutes: 4, solution: solved(`${SARI_SARI_STORE_INTRO_BODY}    <p>Water: H<sub>2</sub>O</p>\n`) },
  ...
  "stock-list-thead": { estimatedMinutes: 5, solution: solvedSlot(STOCK_LIST_ELEMENT_SLOT, "<thead><tr><th></th></tr></thead>") },
  "stock-list-thead-text": { estimatedMinutes: 4, solution: solved(STOCK_LIST_ELEMENT_BODY.replace("<th></th>", "<th>Medicine</th>")) },
  ...
  "id-application-tbody": { estimatedMinutes: 5, solution: solvedSlot(ID_APPLICATION_ELEMENT_SLOT, "<tbody><tr><td></td></tr></tbody>") },
  "id-application-tbody-text": { estimatedMinutes: 4, solution: solved(ID_APPLICATION_ELEMENT_BODY.replace("<td></td>", "<td>Maria Santos</td>")) },
  ...
  "store-price-tfoot": { estimatedMinutes: 5, solution: solvedSlot(STORE_PRICE_ELEMENT_SLOT, "<tfoot><tr><td></td></tr></tfoot>") },
  "store-price-tfoot-text": { estimatedMinutes: 4, solution: solved(STORE_PRICE_ELEMENT_BODY.replace("<td></td>", "<td>Total: ₱100.00</td>")) },
  ...
  "health-centre-u-text": { estimatedMinutes: 4, solution: solved(`${HEALTH_CENTRE_INTRO_BODY}    <u>vacine</u>\n`) },
  ...
  "centre-notice-root": { estimatedMinutes: 4, solution: solvedSlot(CENTRE_NOTICE_ROOT_SLOT, "<section></section>") },
  "centre-notice-h2-1": { estimatedMinutes: 4, solution: solved(CENTRE_NOTICE_B2) },
  "centre-notice-p-2": { estimatedMinutes: 4, solution: solved(CENTRE_NOTICE_B3) },
...
const s54 = (step: Omit<Step, "index" | "kind" | "projectId">): Step => {
  const reference = references[step.id as keyof typeof references];
  return { ...step, ...reference, index: ++n, kind: "web", projectId: PROJECT_54_ID };
};
...
export const htmlCourse: Course = {
  ...
  projects: [
    ...
    { id: PROJECT_30_ID, title: "Water Refill Label" },
  ...
  steps: [
    ...
    s28({ id: "health-center-notice-copy", task: "Add the opening sentence below the heading.", inputMode: "guided", files: solved(HEALTH_CENTER_NOTICE_HEADING_BODY), activeFile: "index.html", highlightToken: "</h2>", tests: [{ id: "health-center-notice-copy-text", kind: "text-equals", selector: "p", value: "The notice names the mosquito that can spread dengue.", label: "The opening sentence is on the page" }], hints: [{ level: 1, text: "Add one paragraph below the heading." }, { level: 2, text: "Write the sentence shown in the instruction." }], xp: 40 }),
    s28({ id: "health-center-notice-i", task: "Add a place for the mosquito's scientific name.", inputMode: "tap-to-build", files: { "index.html": HEALTH_CENTER_NOTICE_ELEMENT_SLOT.page, "styles.css": "" }, activeFile: "index.html", slotLine: HEALTH_CENTER_NOTICE_ELEMENT_SLOT.slotLine, blocks: ["<i></i>","<p></p>","<span></span>","<em></em>"], correctBlock: "<i></i>", conceptIds: ["i-element"], tests: [{ id: "health-center-notice-i-exists", kind: "exists", selector: "i", label: "The scientific name has a place" }], hints: [{ level: 1, text: "Use the element for text set apart from the surrounding sentence." }, { level: 2, text: "Use i for the scientific name." }], xp: 50 }),
    s28({ id: "health-center-notice-i-text", task: "Write Aedes aegypti inside it.", inputMode: "guided", files: solved(HEALTH_CENTER_NOTICE_ELEMENT_BODY), activeFile: "index.html", highlightToken: "<i></i>", tests: [{ id: "health-center-notice-i-text-set", kind: "text-equals", selector: "i", value: "Aedes aegypti", label: "The scientific name is complete" }], hints: [{ level: 1, text: "Write the two-word scientific name inside the tags." }, { level: 2, text: "Use Aedes aegypti exactly." }], xp: 40 }),
    ...
    s29({ id: "jeepney-route-copy", task: "Add the opening sentence below the heading.", inputMode: "guided", files: solved(JEEPNEY_ROUTE_HEADING_BODY), activeFile: "index.html", highlightToken: "</h2>", tests: [{ id: "jeepney-route-copy-text", kind: "text-equals", selector: "p", value: "The route name should be easy to spot without adding extra importance.", label: "The opening sentence is on the page" }], hints: [{ level: 1, text: "Add one paragraph below the heading." }, { level: 2, text: "Write the sentence shown in the instruction." }], xp: 40 }),
    s29({ id: "jeepney-route-b", task: "Add a place for a route name that readers should notice.", inputMode: "tap-to-build", files: { "index.html": JEEPNEY_ROUTE_ELEMENT_SLOT.page, "styles.css": "" }, activeFile: "index.html", slotLine: JEEPNEY_ROUTE_ELEMENT_SLOT.slotLine, blocks: ["<b></b>","<p></p>","<span></span>","<strong></strong>"], correctBlock: "<b></b>", conceptIds: ["b-element"], tests: [{ id: "jeepney-route-b-exists", kind: "exists", selector: "b", label: "The route name has a place" }], hints: [{ level: 1, text: "Use the element that draws attention without adding importance." }, { level: 2, text: "Use b for the route name." }], xp: 50 }),
    s29({ id: "jeepney-route-b-text", task: "Write Divisoria inside it.", inputMode: "guided", files: solved(JEEPNEY_ROUTE_ELEMENT_BODY), activeFile: "index.html", highlightToken: "<b></b>", tests: [{ id: "jeepney-route-b-text-set", kind: "text-equals", selector: "b", value: "Divisoria", label: "The route name is visible" }], hints: [{ level: 1, text: "Write the destination inside the tags." }, { level: 2, text: "Use Divisoria exactly." }], xp: 40 }),
    s30({ id: "sari-sari-store-title", task: "Start the water refill label with a smaller heading.", inputMode: "tap-to-build", files: { "index.html": SARI_SARI_STORE_TITLE_SLOT.page, "styles.css": "" }, activeFile: "index.html", slotLine: SARI_SARI_STORE_TITLE_SLOT.slotLine, blocks: ["<h2></h2>", "<p></p>", "<h1></h1>", "<sub></sub>"], correctBlock: "<h2></h2>", tests: [{ id: "sari-sari-store-title-exists", kind: "exists", selector: "h2", label: "The page has a heading" }], hints: [{ level: 1, text: "Add a level-two heading in the blank line." }, { level: 2, text: "Use h2 for this smaller heading." }], xp: 40 }),
    s30({ id: "sari-sari-store-heading", task: "Name the heading Water Refill Label.", inputMode: "guided", files: solved(SARI_SARI_STORE_TITLE_BODY), activeFile: "index.html", highlightToken: "<h2></h2>", tests: [{ id: "sari-sari-store-heading-text", kind: "text-equals", selector: "h2", value: "Water Refill Label", label: "The page has its name" }], hints: [{ level: 1, text: "Write the page name between the heading tags." }, { level: 2, text: "Use Water Refill Label exactly." }], xp: 40 }),
    s30({ id: "sari-sari-store-copy", task: "Add the opening sentence below the heading.", inputMode: "guided", files: solved(SARI_SARI_STORE_HEADING_BODY), activeFile: "index.html", highlightToken: "</h2>", tests: [{ id: "sari-sari-store-copy-text", kind: "text-equals", selector: "p", value: "The label needs the chemical formula for water.", label: "The opening sentence is on the page" }], hints: [{ level: 1, text: "Add one paragraph below the heading." }, { level: 2, text: "Write the sentence shown in the instruction." }], xp: 40 }),
    s30({ id: "sari-sari-store-sub", task: "Add the water formula with an empty subscript.", inputMode: "tap-to-build", files: { "index.html": SARI_SARI_STORE_ELEMENT_SLOT.page, "styles.css": "" }, activeFile: "index.html", slotLine: SARI_SARI_STORE_ELEMENT_SLOT.slotLine, blocks: ["<p>Water: H<sub></sub>O</p>","<p>Water: H2O</p>","<p>Water: H<sup></sup>O</p>","<sub>H2O</sub>"], correctBlock: "<p>Water: H<sub></sub>O</p>", conceptIds: ["sub-element"], tests: [{ id: "sari-sari-store-sub-exists", kind: "exists", selector: "sub", label: "The formula has a subscript" }], hints: [{ level: 1, text: "The lowered number belongs between H and O." }, { level: 2, text: "Use sub around the empty place for the number." }], xp: 50 }),
    s30({ id: "sari-sari-store-sub-text", task: "Write 2 inside the subscript.", inputMode: "guided", files: solved(SARI_SARI_STORE_ELEMENT_BODY), activeFile: "index.html", highlightToken: "<sub></sub>", tests: [{ id: "sari-sari-store-sub-text-set", kind: "text-equals", selector: "sub", value: "2", label: "The formula has the lowered number 2" }], hints: [{ level: 1, text: "Write the number inside the sub tags." }, { level: 2, text: "Use 2 exactly." }], xp: 40 }),
    ...
    s34({ id: "stock-list-copy", task: "Add the opening sentence below the heading.", inputMode: "guided", files: solved(STOCK_LIST_HEADING_BODY), activeFile: "index.html", highlightToken: "</h2>", tests: [{ id: "stock-list-copy-text", kind: "text-equals", selector: "p", value: "The table separates medicine headings from stock rows.", label: "The opening sentence is on the page" }], hints: [{ level: 1, text: "Add one paragraph below the heading." }, { level: 2, text: "Write the sentence shown in the instruction." }], xp: 40 }),
    s34({ id: "stock-list-thead", task: "Add a header row inside the table head.", inputMode: "tap-to-build", files: { "index.html": STOCK_LIST_ELEMENT_SLOT.page, "styles.css": "" }, activeFile: "index.html", slotLine: STOCK_LIST_ELEMENT_SLOT.slotLine, blocks: ["<thead><tr><th></th></tr></thead>","<tbody><tr><td></td></tr></tbody>","<tr><td></td></tr>","<thead></thead>"], correctBlock: "<thead><tr><th></th></tr></thead>", conceptIds: ["thead-element"], tests: [{ id: "stock-list-thead-exists", kind: "exists", selector: "table > thead > tr > th", label: "The table has a valid header row" }], hints: [{ level: 1, text: "A table head contains a row, and the row contains a heading cell." }, { level: 2, text: "Nest tr and th inside thead." }], xp: 50 }),
    s34({ id: "stock-list-thead-text", task: "Name the heading cell Medicine.", inputMode: "guided", files: solved(STOCK_LIST_ELEMENT_BODY), activeFile: "index.html", highlightToken: "<th></th>", tests: [{ id: "stock-list-thead-text-set", kind: "text-equals", selector: "thead th", value: "Medicine", label: "The table head names the medicine column" }], hints: [{ level: 1, text: "Write the column name inside the th tags." }, { level: 2, text: "Use Medicine exactly." }], xp: 40 }),
    ...
    s35({ id: "id-application-copy", task: "Add the opening sentence below the heading.", inputMode: "guided", files: solved(ID_APPLICATION_HEADING_BODY), activeFile: "index.html", highlightToken: "</h2>", tests: [{ id: "id-application-copy-text", kind: "text-equals", selector: "p", value: "The table body will hold one row for each applicant.", label: "The opening sentence is on the page" }], hints: [{ level: 1, text: "Add one paragraph below the heading." }, { level: 2, text: "Write the sentence shown in the instruction." }], xp: 40 }),
    s35({ id: "id-application-tbody", task: "Add an applicant row inside the table body.", inputMode: "tap-to-build", files: { "index.html": ID_APPLICATION_ELEMENT_SLOT.page, "styles.css": "" }, activeFile: "index.html", slotLine: ID_APPLICATION_ELEMENT_SLOT.slotLine, blocks: ["<tbody><tr><td></td></tr></tbody>","<thead><tr><th></th></tr></thead>","<tr><th></th></tr>","<tbody></tbody>"], correctBlock: "<tbody><tr><td></td></tr></tbody>", conceptIds: ["tbody-element"], tests: [{ id: "id-application-tbody-exists", kind: "exists", selector: "table > tbody > tr > td", label: "The table has a valid body row" }], hints: [{ level: 1, text: "A table body contains a row, and the row contains a data cell." }, { level: 2, text: "Nest tr and td inside tbody." }], xp: 50 }),
    s35({ id: "id-application-tbody-text", task: "Write Maria Santos in the applicant cell.", inputMode: "guided", files: solved(ID_APPLICATION_ELEMENT_BODY), activeFile: "index.html", highlightToken: "<td></td>", tests: [{ id: "id-application-tbody-text-set", kind: "text-equals", selector: "tbody td", value: "Maria Santos", label: "The table body names the applicant" }], hints: [{ level: 1, text: "Write the applicant name inside the td tags." }, { level: 2, text: "Use Maria Santos exactly." }], xp: 40 }),
    ...
    s36({ id: "store-price-copy", task: "Add the opening sentence below the heading.", inputMode: "guided", files: solved(STORE_PRICE_HEADING_BODY), activeFile: "index.html", highlightToken: "</h2>", tests: [{ id: "store-price-copy-text", kind: "text-equals", selector: "p", value: "The table footer will hold the price summary.", label: "The opening sentence is on the page" }], hints: [{ level: 1, text: "Add one paragraph below the heading." }, { level: 2, text: "Write the sentence shown in the instruction." }], xp: 40 }),
    s36({ id: "store-price-tfoot", task: "Add a summary row inside the table footer.", inputMode: "tap-to-build", files: { "index.html": STORE_PRICE_ELEMENT_SLOT.page, "styles.css": "" }, activeFile: "index.html", slotLine: STORE_PRICE_ELEMENT_SLOT.slotLine, blocks: ["<tfoot><tr><td></td></tr></tfoot>","<tbody><tr><td></td></tr></tbody>","<tr><th></th></tr>","<tfoot></tfoot>"], correctBlock: "<tfoot><tr><td></td></tr></tfoot>", conceptIds: ["tfoot-element"], tests: [{ id: "store-price-tfoot-exists", kind: "exists", selector: "table > tfoot > tr > td", label: "The table has a valid summary row" }], hints: [{ level: 1, text: "A table footer contains a row, and the row contains a data cell." }, { level: 2, text: "Nest tr and td inside tfoot." }], xp: 50 }),
    s36({ id: "store-price-tfoot-text", task: "Write Total: ₱100.00 in the summary cell.", inputMode: "guided", files: solved(STORE_PRICE_ELEMENT_BODY), activeFile: "index.html", highlightToken: "<td></td>", tests: [{ id: "store-price-tfoot-text-set", kind: "text-equals", selector: "tfoot td", value: "Total: ₱100.00", label: "The table footer shows the total" }], hints: [{ level: 1, text: "Write the summary inside the td tags." }, { level: 2, text: "Use Total: ₱100.00 exactly." }], xp: 40 }),
    ...
    s37({ id: "health-centre-copy", task: "Add the opening sentence below the heading.", inputMode: "guided", files: solved(HEALTH_CENTRE_HEADING_BODY), activeFile: "index.html", highlightToken: "</h2>", tests: [{ id: "health-centre-copy-text", kind: "text-equals", selector: "p", value: "The draft notice contains one misspelled word.", label: "The opening sentence is on the page" }], hints: [{ level: 1, text: "Add one paragraph below the heading." }, { level: 2, text: "Write the sentence shown in the instruction." }], xp: 40 }),
    s37({ id: "health-centre-u", task: "Add a place for the misspelled word.", inputMode: "tap-to-build", files: { "index.html": HEALTH_CENTRE_ELEMENT_SLOT.page, "styles.css": "" }, activeFile: "index.html", slotLine: HEALTH_CENTRE_ELEMENT_SLOT.slotLine, blocks: ["<u></u>","<p></p>","<div></div>","<mark></mark>"], correctBlock: "<u></u>", conceptIds: ["u-element"], tests: [{ id: "health-centre-u-exists", kind: "exists", selector: "u", label: "The annotated word has a place" }], hints: [{ level: 1, text: "Use the element for a non-textual annotation such as a spelling mark." }, { level: 2, text: "Use u for the misspelled word." }], xp: 50 }),
    s37({ id: "health-centre-u-text", task: "Write vacine inside it as the draft's misspelling.", inputMode: "guided", files: solved(HEALTH_CENTRE_ELEMENT_BODY), activeFile: "index.html", highlightToken: "<u></u>", tests: [{ id: "health-centre-u-text-set", kind: "text-equals", selector: "u", value: "vacine", label: "The misspelled word is annotated" }], hints: [{ level: 1, text: "Write the draft word inside the u tags." }, { level: 2, text: "Use vacine exactly." }], xp: 40 }),
    ...
    s43({ id: "fare-table-h2-1", task: "Add the heading inside the notice, and write Tricycle Fare Update in it.", inputMode: "guided", files: solved(FARE_TABLE_B1), activeFile: "index.html", highlightToken: "<article>", tests: [{ id: "fare-table-h2-1-text", kind: "text-equals", selector: "article h2", value: "Tricycle Fare Update", label: "The heading shows the notice heading" }], hints: [{ level: 1, text: "Find the notice you already added, and work inside the notice." }, { level: 2, text: "Use h2 tags, and write Tricycle Fare Update between them." }], xp: 45 }),
    s43({ id: "fare-table-p-2", task: "Add the paragraph inside the notice, and write Effective immediately, all rates increased by Php5. in it.", inputMode: "guided", files: solved(FARE_TABLE_B2), activeFile: "index.html", highlightToken: "<article>", tests: [{ id: "fare-table-p-2-text", kind: "text-equals", selector: "article p", value: "Effective immediately, all rates increased by Php5.", label: "The paragraph shows one sentence of detail" }], hints: [{ level: 1, text: "Find the notice you already added, and work inside the notice." }, { level: 2, text: "Use p tags, and write Effective immediately, all rates increased by Php5. between them." }], xp: 45 }),
    ...
    s44({ id: "order-slip-h2-1", task: "Add the heading inside the section, and write Bakery Order in it.", inputMode: "guided", files: solved(ORDER_SLIP_B1), activeFile: "index.html", highlightToken: "<section>", tests: [{ id: "order-slip-h2-1-text", kind: "text-equals", selector: "section h2", value: "Bakery Order", label: "The heading shows the list heading" }], hints: [{ level: 1, text: "Find the section you already added, and work inside the section." }, { level: 2, text: "Use h2 tags, and write Bakery Order between them." }], xp: 45 }),
    ...
    s44({ id: "order-slip-li-3", task: "Add the list item inside the list, and write Dozen Baguettes in it.", inputMode: "guided", files: solved(ORDER_SLIP_B3), activeFile: "index.html", highlightToken: "<ul>", tests: [{ id: "order-slip-li-3-text", kind: "text-equals", selector: "section ul li", value: "Dozen Baguettes", label: "The list item shows the first item" }], hints: [{ level: 1, text: "Find the list you already added, and work inside the list." }, { level: 2, text: "Use li tags, and write Dozen Baguettes between them." }], xp: 45 }),
    s44({ id: "order-slip-li-4", task: "Add the list item inside the list, and write Sugar Cookies in it.", inputMode: "guided", files: solved(ORDER_SLIP_B4), activeFile: "index.html", highlightToken: "<ul>", tests: [{ id: "order-slip-li-4-text", kind: "text-equals", selector: "section ul li:nth-of-type(2)", value: "Sugar Cookies", label: "The list item shows the second item" }], hints: [{ level: 1, text: "Find the list you already added, and work inside the list." }, { level: 2, text: "Use li tags, and write Sugar Cookies between them." }], xp: 45 }),
    s44({ id: "order-slip-li-5", task: "Add the list item inside the list, and write Croissant in it.", inputMode: "guided", files: solved(ORDER_SLIP_B5), activeFile: "index.html", highlightToken: "<ul>", tests: [{ id: "order-slip-li-5-text", kind: "text-equals", selector: "section ul li:nth-of-type(3)", value: "Croissant", label: "The list item shows the third item" }], hints: [{ level: 1, text: "Find the list you already added, and work inside the list." }, { level: 2, text: "Use li tags, and write Croissant between them." }], xp: 45 }),
    ...
    s45({ id: "refill-station-h2-1", task: "Add the heading inside the section, and write Water Refill Station in it.", inputMode: "guided", files: solved(REFILL_STATION_B1), activeFile: "index.html", highlightToken: "<section>", tests: [{ id: "refill-station-h2-1-text", kind: "text-equals", selector: "section h2", value: "Water Refill Station", label: "The heading shows the section heading" }], hints: [{ level: 1, text: "Find the section you already added, and work inside the section." }, { level: 2, text: "Use h2 tags, and write Water Refill Station between them." }], xp: 45 }),
    ...
    s54({ id: "centre-notice-root", task: "Start the notice card. Add the box that holds everything else.", inputMode: "tap-to-build", files: { "index.html": CENTRE_NOTICE_ROOT_SLOT.page, "styles.css": "" }, activeFile: "index.html", slotLine: CENTRE_NOTICE_ROOT_SLOT.slotLine, blocks: ["<section></section>","<div></div>","<p></p>","<span></span>"], correctBlock: "<section></section>", tests: [{ id: "centre-notice-root-exists", kind: "exists", selector: "section", label: "The notice card has its outer box" }], hints: [{ level: 1, text: "Add the element that groups everything else in this notice card." }, { level: 2, text: "Use section tags for the outer box." }], xp: 40 }),
    s54({ id: "centre-notice-h2-1", task: "Add the heading inside the section, and write Vaccination Alert in it.", inputMode: "guided", files: solved(CENTRE_NOTICE_B1), activeFile: "index.html", highlightToken: "<section>", tests: [{ id: "centre-notice-h2-1-text", kind: "text-equals", selector: "section h2", value: "Vaccination Alert", label: "The heading shows the card heading" }], hints: [{ level: 1, text: "Find the section you already added, and work inside the section." }, { level: 2, text: "Use h2 tags, and write Vaccination Alert between them." }], xp: 45 }),
    s54({ id: "centre-notice-p-2", task: "Add the paragraph inside the section, and write All residents welcome for free shots. Next session tomorrow at 8 AM. in it.", inputMode: "guided", files: solved(CENTRE_NOTICE_B2), activeFile: "index.html", highlightToken: "<section>", tests: [{ id: "centre-notice-p-2-text", kind: "text-equals", selector: "section p", value: "All residents welcome for free shots. Next session tomorrow at 8 AM.", label: "The paragraph shows one sentence of detail" }], hints: [{ level: 1, text: "Find the section you already added, and work inside the section." }, { level: 2, text: "Use p tags, and write All residents welcome for free shots. Next session tomorrow at 8 AM. between them." }], xp: 45 }),
