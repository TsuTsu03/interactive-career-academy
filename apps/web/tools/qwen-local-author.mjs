// Local-model authoring for the Command Line and Git course.
//
// Called by run-qwen-v2-loop.ps1 for one batch of five steps. The commands,
// checks, starting files, step ids, and concepts come from
// tools/cli-git-plan.mjs and never from the model. The model writes only the
// learner-facing words: each step's task, two hints, and a time estimate.
// Everything it returns is plain JSON, validated here, and serialised into
// content/cli-git-course.ts. `npm run check:content` then replays every
// command in a throwaway folder before the batch is accepted.
...
async function requestModel({ endpoint, model, system, user, schema, retrying, receiptPath }) {
  const body = {
    ...
    stream: true,
    response_format: { type: "json_schema", json_schema: { name: "cli_git_batch", strict: true, schema } },
  };
...
export async function authorLocalBatch({ root, courseFile, logFile, projectId, endpoint, model, promptPath, receiptPath }) {
  ...
  if (!course) throw Error("cli-git is not registered.");

  const project = cliGitProjects.find((item) => item.id === projectId);
  if (!project) throw Error(`Unknown cli-git project ${projectId}.`);
  ...
  const addition = `\n// Validated local authoring batch: ${project.id}.\ncliGitCourse.steps.push(...(${JSON.stringify(generated, null, 2)} satisfies typeof cliGitCourse.steps));\n`;
  fs.writeFileSync(resolve(courseFile), fs.readFileSync(resolve(courseFile), "utf8") + addition);
  const date = new Date().toISOString().slice(0, 10);
  fs.appendFileSync(resolve(logFile), `\n- ${date}: Local Qwen wrote the lesson text for cli-git/${project.id}, steps ${generated[0].index}-${generated.at(-1).index}; commands and checks come from tools/cli-git-plan.mjs and were replayed by check:content before acceptance.\n`);
