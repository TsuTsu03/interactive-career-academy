...
export function AccountScreen() {
  ...
  return <div className="flex min-h-[100dvh] flex-col bg-background text-on-background"><ProductNav current="account" /><main className="mx-auto w-full max-w-[980px] flex-1 px-margin-mobile py-8 md:px-margin-desktop md:py-12">
    ...
    <section className={`mt-8 rounded-2xl border p-5 sm:p-7 ${signedIn ? "border-acid/40" : configured ? "border-hairline" : "border-gold/40"} bg-panel`}>
      ...
      <p className="mt-3 text-[15px] leading-relaxed text-ash">{signedIn ? "Your account can store progress, current lesson code, capstone links, and certificate records. Sync remains explicit so another device never silently replaces this browser's code." : configured ? "Continue with GitHub or Google, or ask for a one-time email link. Any of the three both registers you and signs you in, and none of them uses a password." : "Local learning remains available. Add the documented Supabase environment values and migration before account features can connect."}</p>
    </section>

    {!signedIn ? <div className="mt-6 grid gap-4 md:grid-cols-2">
      <section className="rounded-2xl border border-hairline bg-panel p-5 sm:p-6">
        <h2 className="font-display text-[22px] font-bold text-chalk">Sign up or sign in</h2>
